...
export default function BottomNav({ lang }: { lang: string }) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 h-16 bg-black/60 backdrop-blur-xl border-t border-white/5 flex items-center justify-around px-2 z-50 transition-all">
      {items.map((item) => (
        <NavLink
          key={item.path}
          to={item.path}
